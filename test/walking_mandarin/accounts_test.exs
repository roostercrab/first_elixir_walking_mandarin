defmodule WalkingMandarin.AccountsTest do
  ...
  describe "get_users_by_email_and_password/1" do
    test "does not return the users if the email does not exist" do
  ...
  describe "register_users/1" do
    ...
    test "registers user with a hashed password" do
      ...
      {:ok, users} = Accounts.register_users(%{email: email, password: valid_users_password()})
  ...
  describe "apply_users_email/3" do
    ...
    test "validates maximum value for e-mail for security", %{users: users} do
      too_long = String.duplicate("db", 100)
    ...
    test "validates e-mail uniqueness", %{users: users} do
      %{email: email} = users_fixture()
    ...
    test "applies the e-mail without persisting it", %{users: users} do
      email = unique_users_email()
  ...
  describe "update_users_email/2" do
    ...
    test "updates the e-mail with a valid token", %{users: users, token: token, email: email} do
    ...
    test "does not update e-mail with invalid token", %{users: users} do
    ...
    test "does not update e-mail if users e-mail changed", %{users: users, token: token} do
      assert Accounts.update_users_email(%{users | email: "current@example.com"}, token) == :error
      assert Repo.get!(Users, users.id).email == users.email
      assert Repo.get_by(UsersToken, users_id: users.id)
    end

    test "does not update e-mail if token expired", %{users: users, token: token} do
  ...
  describe "confirm_users/2" do
    ...
    test "confirms the e-mail with a valid token", %{users: users, token: token} do
    ...
    test "does not confirm e-mail if token expired", %{users: users, token: token} do
      {1, nil} = Repo.update_all(UsersToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
  ...
  describe "get_users_by_reset_password_token/2" do
    setup do
      users = users_fixture()
  ...
  describe "reset_users_password/3" do
    setup do
      %{users: users_fixture()}
    end
