defmodule WalkingMandarin.Accounts do
  @moduledoc """
  ...
  alias WalkingMandarin.Accounts.{Users, UsersToken, UsersNotifier}
  ...
  def change_users_registration(%Users{} = users, attrs \\ %{}) do
    Users.registration_changeset(users, attrs)
  end

  ## Settings

  @doc """
  Returns an `%Ecto.Changeset{}` for changing the users e-mail.
  ...
  @doc """
  Updates the users e-mail in token.
  ...
  defp users_email_multi(users, email, context) do
    changeset = users |> Users.email_changeset(%{email: email}) |> Users.confirm_changeset()
  ...
  @doc """
  Delivers the update e-mail instructions to the given users.
  ...
  def change_users_password(users, attrs \\ %{}) do
    Users.password_changeset(users, attrs)
  end
  ...
  @doc """
  Delivers the confirmation e-mail instructions to the given users.

  ## Examples

      iex> deliver_users_confirmation_instructions(users, &Routes.users_confirmation_url(conn, :confirm, &1))
      {:ok, %{to: ..., body: ...}}

      iex> deliver_users_confirmation_instructions(confirmed_users, &Routes.users_confirmation_url(conn, :confirm, &1))
  ...
  @doc """
  Delivers the reset password e-mail to the given users.
